...
<template>
  <div class="py-4">
    ...
    <div v-if="cart.wishlist.length === 0" class="text-center py-16 card">
      ...
      <p class="text-slate-500 mb-6">Save your favorite items here for later!</p>
      <RouterLink to="/products" class="btn-accent">Browse Products</RouterLink>
    </div>

    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="product in cart.wishlist" :key="product.id" class="card relative overflow-hidden">
        <button
        ...
        <div class="p-4">
          ...
          <div class="text-xl font-bold text-emerald-600 mb-1">${{ product.price.toFixed(2) }}</div>
          <div
            class="text-xs mb-3"
          ...
          <button
            @click="cart.addToCart(product); cart.removeFromWishlist(product.id)"
