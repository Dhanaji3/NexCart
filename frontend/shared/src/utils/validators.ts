...
export function validatePhone(phone: string): ValidationResult {
  ...
  if (!phoneRegex.test(phone)) return { valid: false, message: 'Invalid phone number format' }
  return { valid: true }
}
...
export function validateZipCode(zip: string): ValidationResult {
  ...
  if (!zipRegex.test(zip)) return { valid: false, message: 'Invalid zip code format' }
  return { valid: true }
}
...
export function validatePrice(price: number): ValidationResult {
  if (price === null || price === undefined) return { valid: false, message: 'Price is required' }
